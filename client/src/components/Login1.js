...
function Login1({store}){
    ...
    function handleForm(event) {
        ...
        const pw=pass.value;
        console.log(un+": "+pw);
        axios.get('http://localhost:1011/checkOwner',{params:{
            un: un,
            pw: pw
        }}).then((response)=>{
    ...
    if((result===null || result==="fail" || result==="Fail") && (store.getState().AuthReducer[0]===null))
    {
        return (
            <div className='App-login'>
                <h3 style={{textShadow: "1px 1px 5px blue, -1px -1px 2px black" , marginBottom:"20px", marginTop:"60px"}}>Login Page For Admin</h3>
                <div>
